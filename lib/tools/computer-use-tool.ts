...
async function reviewComputerAppApproval(options, toolCtx, approval, params) {
  ...
  return {
    ...
    decision,
    reason: decision?.reason || "auto approval denied computer app access",
  };
}
    ...
    if (autoApproval.allowed) {
      ...
      return {
        ...
        confirmation: {
          ...
          reason: autoApproval.reason || autoApproval.decision?.reason,
...
export function createComputerUseTool(options = {}) {
  return {
    ...
    isEnabledForAgentConfig: options.isEnabledForAgentConfig,
